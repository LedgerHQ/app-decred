    ...
    BEGIN_TRY {
        TRY {
            ...
            if (DISPATCHER_DATA_IN[dispatched]) {
                if (G_io_apdu_buffer[ISO_OFFSET_LC] == 0x00 || btchip_context_D.inLength - 5 == 0) {
...
// common ux
        ...
        sendSW:
            ...
            G_io_apdu_buffer[btchip_context_D.outLength + 1] = (btchip_context_D.sw & 0xff);
...
void app_main(void) {
    explicit_bzero(G_io_apdu_buffer, 255);  // paranoia
    ...
    for (;;) {
        ...
        btchip_context_D.inLength = io_exchange(CHANNEL_APDU | btchip_context_D.io_flags,
