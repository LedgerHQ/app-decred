...
uint8_t prepare_message_signature() {
    ...
    cx_hash((cx_hash_t *) &btchip_context_D.messageHashWitness.header,
            CX_LAST,
            (const unsigned char *) vars.tmp.fullAmount,
            0,
            buffer,
            32);

#ifdef HAVE_NBGL
    snprintf(vars.tmp.fullAddress, sizeof(vars.tmp.fullAddress), "%.*H", 32, buffer);
#else
...
uint8_t prepare_single_output() {
    ...
    if (vars.tmp.fullAddress[0] == 0) {
        ...
        memmove(address + versionSize, btchip_context_D.currentOutput + addressOffset, 20);
    ...
    memmove(vars.tmp.fullAmount, btchip_context_D.shortCoinId, btchip_context_D.shortCoinIdLength);
...
uint8_t prepare_full_output(uint8_t checkOnly) {
    ...
    for (i = 0; i < numberOutputs; i++) {
        ...
        if (btchip_context_D.tmpCtx.output.changeInitialized && !isOpReturn) {
            ...
            if (memcmp(btchip_context_D.currentOutput + offset + addressOffset,
