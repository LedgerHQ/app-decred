...
packets[0] = (
    "e0420000" + hexlify(bytes([int(len(packets[0]) / 2)])).decode("utf-8") + packets[0]
)
result = dongle.exchange(bytearray.fromhex(packets[0]))
...
packets = [
    ...
    "01"
    + "%0.2X" % len(trusted_input)
    + hexlify(trusted_input).decode("utf-8")
    + "00"
...
packets[0] = (
    "e0440000" + hexlify(bytes([int(len(packets[0]) / 2)])).decode("utf-8") + packets[0]
)
result = dongle.exchange(bytearray.fromhex(packets[0]))
...
packets[0] = (
    "e04aFF00" + hexlify(bytes([int(len(packets[0]) / 2)])).decode("utf-8") + packets[0]
)
result = dongle.exchange(bytearray.fromhex(packets[0]))
...
packets[0] = (
    "e0480000" + hexlify(bytes([int(len(packets[0]) / 2)])).decode("utf-8") + packets[0]
)
result = dongle.exchange(bytearray.fromhex(packets[0]))
