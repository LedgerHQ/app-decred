...
@pytest.mark.order(6)
def test_1to2_reject_get_pubkey(scenario_navigator: NavigateWithScenario):
    # The goal of the whole test sequence (test_1to2_reject_*) is to reject a transaction
    # To achieve that the address needs to be approved first
    # Hence the snapshot containing "address verified" screen for this one test which may be confusing : test_1to2_reject_get_pubkey/00002.png
...
# ################# GET TRUSTED INPUT #########################
@pytest.mark.order(7)
def test_1to2_reject_get_trusted_input(backend: BackendInterface):
    packets = [
...
# ################# HASH INPUT START #########################
@pytest.mark.order(8)
def test_1to2_reject_hash_input_start(backend: BackendInterface):
    packets = [
        ...
        "01"
        + "%0.2X" % len(trusted_input.data)
        + hexlify(trusted_input.data).decode("utf-8")
        + "00"
...
@pytest.mark.order(9)
def test_1to2_reject_hash_input_finalize(scenario_navigator: NavigateWithScenario):
    packets = [
